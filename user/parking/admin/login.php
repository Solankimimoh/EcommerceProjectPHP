...
if ($_POST)
{
    ...
    $loginq=mysql_query("SELECT * FROM `guard` WHERE `guard_email`='{$email}' and `guard_password`='{$password}'")or die(mysql_error());
    $data= mysql_fetch_array($loginq);
    if ($data['is_admin']=='Yes')
    {
        $_SESSION['admin_id']=$data['user_id'];
        $_SESSION['admin_name']=$data['user_name'];
        header("location:display-master.php");
    }
        else if($data['is_admin']=='No') 
    {
        
            $_SESSION['guard_id']=$data[0];
        $_SESSION['guard_name']=$data[1];
        header("location:../guard/parkingdispaly.php");
    }
    else
    {
