...
$ominutes=floor(($diff_seconds%3600)/60);
$ocharg=
...
$totalfinalwithcurrent= floor($diftotaltime/3600).'H:'.floor(($diftotaltime%3600)/60).'M';
 echo "Old Time".$stat;
 echo "<br>Charge".$stat*30;
  
 $newchrg="<br><br>new time".$totalfinalwithcurrent;
 echo "<br>Total Charge".$totalfinalwithcurrent*30/60;   
 
//$to = time($edit_data['reservation_date'] - $time1);
// $from = time($edit_data['reservation_date'] - $time2);
...
//echo $final_time;
//$minutes = round(abs($time1 + $time2) / 60,2);
//$seconds = abs($time1 - $time2) % 60;
